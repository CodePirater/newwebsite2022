...
#filtered impeach by results that only showed "No"
```
...
impeach %>% 
  filter(for_impeachment == "YES",
         p16winningparty == "R")
#filtered impeach by those who said no to impeaching trump and those who voted trump in 2016
```
...
#filtered impeach for those who voted yes for impeaching trump during the date Semptember 2019 by sorting for the year 2019 and the month September
```
...
impeach %>% 
  filter(for_impeachment == "YES",
         p16winningparty == "D",
         clinton_percent > 70 )
#filtering for yes vote to impeach and voted for Clinton %
```
...
impeach %>% 
  filter(for_impeachment == "NO") %>% 
  filter(state == "NJ")
#filtered for members of congress from New Jersey who voted no to impeachment
```
...
#filtered for members of congress who voted yes to impeachment in 2017
```
...
impeach %>% 
  filter(gdp_above_national == "BELOW",
         for_impeachment == "NO") %>% 
  nrow()
#I got 19 by filtering impeach by gdp above national below the average and sorting for only those who said no to impeaching trump
